...
def run_test(args, eval_only):#run_test(input_dict, testing_region_name_list_loc = None, eval_only = False):
    ...
    # print(args.testing_image_names)
    # exit()
    ...
    # output_loc = input_dict["Log_File"] + "/" + input_dict["Exp_Name"] + "/Detailed_Output"
    output_loc = args.logs_dir + "/Detailed_Output"
    main_eval_region.regional_eval(args, output_loc, quick_mode=(use_quick_mode))

    print("Updating Output Table...")
    if eval_only == False:
        main_eval_region.multi_region_merge(args.logs_dir, "Detailed_Output", output_dir_name="Full_Summary")
