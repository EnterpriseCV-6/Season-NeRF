...
def get_opts():
    ...
    parser.add_argument('--tf', type=str, required=True,
                        help="Month and Day in MM/DD format.")
...
def load_t_nerf(args, file_loc, model_name = "Final_Model.nn"):
    thenet = T_NeRF(args.fc_units, args.number_low_frequency_cases)
    thenet.load_state_dict(t.load(file_loc + "/" + model_name, map_location=t.device("cpu")))
...
def load_model(file_loc):
    ...
    a_t_nerf = load_t_nerf(network_args, file_loc)
    return a_t_nerf, network_args

def parse_time(time_str):
    ans = datetime.datetime.strptime(time_str, "%m/%d")
    start_of_year = datetime.datetime.strptime("01/01", "%m/%d")

    return (ans - start_of_year).days * 1. / 365
...
def _main():
    ...
    view = args.VA
    sun = args.SA
    time = parse_time(args.tf)
    out_img_size = args.Output_Size
    use_exact_solar = args.exact_shadow
    ...
    out_img = imgs["Season_Adj_Img"] * imgs["Shadow_Adjust"]
    plt.imshow(out_img)
    plt.show()
