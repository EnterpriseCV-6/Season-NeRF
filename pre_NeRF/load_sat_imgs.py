...
def load_sat_imgs(site_name, img_location, rpc_location):
    # print(site_name, img_location, rpc_location)
    print("Loading Images")
    return load_imgs_from_location_type_3(img_location, rpc_location, site_name)




def load_imgs_from_location_type_3(img_location, IMD_location, region_id):
    img_list = []
    ...
    for i in range(len(onlyfiles)):
        ...
        if len(file_name) == 4 and (file_name[0] + "_" + file_name[1]) == region_id:
            ...
            test_sat_img.load_rpc_from_tif(img_location + "/" +  onlyfiles[i])
